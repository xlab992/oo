...
def add_channels_to_m3u(channels, m3u_file):
    ...
    # Verifica se il file M3U esiste, altrimenti crealo con l'intestazione
    if not os.path.exists(m3u_file):
        with open(m3u_file, 'w', encoding='utf-8') as f:
            f.write("#EXTM3U\n")
        print(f"File M3U non trovato, creato nuovo file: {m3u_file}")
    # Verifica che il file esista
    elif not os.path.exists(m3u_file): # Questo elif non verrà mai raggiunto a causa del controllo precedente
